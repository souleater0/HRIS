...
class Leave_model extends CI_Model {
    ...
    function select_leave()
    {
        ...
        $this->db->select("leave_tbl.*,staff_tbl.pic,staff_tbl.staff_name,staff_tbl.mobile,staff_tbl.email,department_tbl.department_name,leavetype_tbl.leavetype_name");
        ...
        $this->db->join("leavetype_tbl",'leavetype_tbl.id=leave_tbl.leave_type_id');
        $qry=$this->db->get();
        if($qry->num_rows()>0)$this->db->join("leavetype_tbl",'leavetype_tbl.id=leave_tbl.leave_type_id');
        {
    ...
    function select_leave_byStaffID($staffid)
    {
        ...
        $this->db->select("leave_tbl.*,staff_tbl.staff_name,staff_tbl.address,staff_tbl.mobile,staff_tbl.email,department_tbl.department_name,leavetype_tbl.leavetype_name");
        $this->db->from("leave_tbl");
        $this->db->join("staff_tbl",'staff_tbl.id=leave_tbl.staff_id');
        $this->db->join("department_tbl",'department_tbl.id=staff_tbl.department_id');
         $this->db->join("leavetype_tbl",'leavetype_tbl.id=leave_tbl.leave_type_id');
